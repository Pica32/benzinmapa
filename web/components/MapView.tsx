  ...
  return {
    ...
    features: stations
      ...
      .map(s => {
        ...
        const isReal = s.price?.source === 'mbenzin.cz';
        ...
        return {
          ...
          properties: {
            ...
            price,
            price_n95: s.price?.natural_95 ?? null,
            price_nafta: s.price?.nafta ?? null,
...
function buildPopupHTML(p: Record<string, any>): string {
  ...
  const priceRows = priceItems
    ...
    .map(([lbl, v]) =>
      p.isReal
        ? `<div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:6px;padding:4px 10px;text-align:center"><div style="font-size:9px;color:#166534;text-transform:uppercase;letter-spacing:.05em">${lbl}</div><div style="font-weight:800;color:#15803d;font-size:14px">${Number(v).toFixed(2).replace('.', ',')} Kč</div></div>`
        : `<div style="background:#f9fafb;border:1px solid #e5e7eb;border-radius:6px;padding:4px 10px;text-align:center"><div style="font-size:9px;color:#9ca3af;text-transform:uppercase;letter-spacing:.05em">${lbl}</div><div style="font-weight:600;color:#9ca3af;font-size:13px">~${Number(v).toFixed(2).replace('.', ',')} Kč</div></div>`
    ).join('');

  const reportedLabel = p.isReal && p.reportedAt ? `Naposledy: ${formatDateCZ(p.reportedAt)}` : '';
  ...
  return `
    <div style="font-family:system-ui,sans-serif;min-width:210px;max-width:260px">
      ...
      <a href="/stanice/${p.id}/" style="display:block;margin-top:6px;background:${p.stale ? '#fef2f2' : '#f3f4f6'};color:${p.stale ? '#dc2626' : '#374151'};text-align:center;padding:7px;border-radius:7px;font-size:11px;font-weight:600;text-decoration:none;border:1px solid ${p.stale ? '#fecaca' : '#e5e7eb'}">
        ${p.stale ? '⚠ Zadat aktuální cenu →' : p.isReal ? 'Detail stanice →' : 'Zadat cenu →'}
      </a>
    </div>`;
}
...
export default function MapView({ stations, fuelType, userLat, userLng }: MapViewProps) {
  ...
  // Keep refs current so map.on('load') always uses latest data
  useEffect(() => {
      ...
      const map = new maplibregl.Map({
        ...
        style: {
          ...
          layers: [
            ...
            // Invisible layers used only for queryRenderedFeatures
            {
              id: 'clusters-query',
              type: 'circle',
              source: 'stations',
  ...
  // Update GeoJSON when stations or fuelType change
  useEffect(() => {
    const map = mapRef.current;
    if (!map) return;
  ...
  useEffect(() => {
    ...
    userMarkerRef.current = new MapLib.Marker({ element: el, anchor: 'center' })
      ...
      .setPopup(new MapLib.Popup().setHTML('<b>Vaše poloha</b>'))
      .addTo(map);
      ...
      <div className="absolute bottom-5 left-3 z-[999] bg-white/95 backdrop-blur-sm rounded-xl shadow-lg p-3 text-xs border border-gray-200 pointer-events-none">
        ...
        {([
          ...
          <div key={color} className="flex items-center gap-2 mb-1">
            <span
              className="w-3.5 h-3.5 rounded-full inline-block border border-white shadow"
