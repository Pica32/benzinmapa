        ...
        <div className="flex items-center justify-between mb-2 flex-wrap gap-2">
          <div className="flex items-center gap-2">
            <span className="text-sm font-semibold text-green-800 dark:text-green-300">
              📍 Nejbližší {FUEL_LABELS[fuelType].toLowerCase()}
            </span>
        ...
        <div className="flex gap-3 overflow-x-auto pb-1 snap-x">
          {nearby.map(s => {
            ...
            return (
              <div
                key={s.id}
                className="flex-shrink-0 snap-start w-44 bg-white dark:bg-gray-700 rounded-xl border border-gray-200 dark:border-gray-600 p-3 shadow-sm"
              >
                ...
                <div className="flex items-center justify-between mb-2">
                  <span className={`text-lg font-black ${isReal ? 'text-green-700 dark:text-green-400' : 'text-gray-500'}`}>
                    {price.toFixed(2).replace('.', ',')}
                  </span>
                  <span className="text-xs text-gray-400">Kč/l</span>
                </div>
