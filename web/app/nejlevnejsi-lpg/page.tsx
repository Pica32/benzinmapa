...
function brandStyleLpg(diff: number) {
...
export default function NejlevnejsiLpgPage() {
  ...
  const stats = getStats();
  const brandStats = getBrandStats('lpg');
        ...
        {brandStats.length > 0 && (
          <section className="mb-10">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
              Průměrná odchylka ceny LPG dané sítě od národního průměru — spočítáno z aktuálních cen.
            </p>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {brandStats.map(({ label, diff, diffLabel, count }) => {
                const { color, bg } = brandStyleLpg(diff);
                return (
